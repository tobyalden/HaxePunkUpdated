 ...
 */
class FramePerfectQuadMotion extends FramePerfectMotion
{
	/**
	 ...
	 */
	public function setMotion(fromX:Float, fromY:Float, controlX:Float, controlY:Float, toX:Float, toY:Float, duration:Int, ?ease:EaseFunction)
	{
		set(fromX, fromY, controlX, controlY, toX, toY);
		_targetInFrames = duration;
		_ease = ease;
		start();
	}
	...
	{
		set(fromX, fromY, controlX, controlY, toX, toY);
		_targetInFrames = Std.int(distance / speed);
		_ease = ease;
		start();
	}
